...
| `pnpm build` | 构建静态站点（输出到 `build/`，含图片优化） |
| `pnpm build:cloudflare` | Cloudflare Workers 构建（输出到 `.svelte-kit/cloudflare/`，含图片优化） |
| `pnpm build:cf-pages` | Cloudflare Pages 静态构建（输出到 `build/`，含图片优化） |
| `pnpm build:netlify` | Netlify 静态构建（输出到 `build/`，含图片优化） |
| `pnpm build:vercel` | Vercel 静态构建（输出到 `build/`，含图片优化） |
| `pnpm build:edgeone` | 腾讯 EdgeOne 全球区构建（输出到 `build/`，含图片优化） |
| `pnpm preview` | 预览构建产物 |
...
本项目为**全量预渲染静态博客**（所有路由 `prerender = true`），因此除 Cloudflare Workers 使用 `adapter-cloudflare` 外，其余平台统一使用 `adapter-static` 输出静态文件到 `build/` 目录。构建时自动执行图片优化（PNG/JPG/WebP → AVIF）。
...
│                  │                              │
│  cf-pages        │  Workers (SSR + Assets)      │
│  netlify         │                              │
│  vercel          │                              │
│  edgeone         │                              │
├──────────────────┴──────────────────────────────┤
│            post-images.js (AVIF 优化)             │
│        OUTPUT_DIR 控制输出目录（默认 build）       │
└─────────────────────────────────────────────────┘
```
...
唯一使用 `@sveltejs/adapter-cloudflare` 的平台，支持 Worker + Static Assets 混合模式。Worker 负责路由和 SSR 回退，静态资源由 Cloudflare 边缘 CDN 直接分发。
...
# 或分步执行
pnpm build:cloudflare    # 输出到 .svelte-kit/cloudflare/
wrangler deploy
```
...
使用 `adapter-static`，输出到 `build/` 目录。静态文件直接部署到 Cloudflare 全球 CDN。
...
[static/_headers](file:///d:/IDE/blog/static/_headers) 和 [static/_redirects](file:///d:/IDE/blog/static/_redirects) 在构建时自动复制到 `build/`，Cloudflare Pages 自动识别并应用。

---

### 三、Netlify

使用 `adapter-static`，输出到 `build/` 目录。通过 `netlify.toml` 配置构建和 HTTP 头。
...
3. Netlify 自动检测 `netlify.toml`，无需手动配置
4. 点击 Deploy

#### 自定义域名

Netlify Dashboard → Site settings → Domain management → Add custom domain → 配置 DNS（CNAME 或 Netlify DNS）

---

### 四、Vercel

使用 `adapter-static`，输出到 `build/` 目录。通过 `vercel.json` 配置构建、HTTP 头和 SPA 回退。
...
1. 推送代码到 GitHub / GitLab / Bitbucket
2. 登录 [Vercel](https://vercel.com/) → Add New → Project → 导入 Git 仓库
3. Vercel 自动检测 SvelteKit 框架和 [vercel.json](file:///d:/IDE/blog/vercel.json) 配置
4. 点击 Deploy

> 每次 `git push` 自动构建并部署。Preview Deployments 为每个 PR 生成预览链接。

#### 自定义域名

Vercel Dashboard → 项目 → Settings → Domains → 添加域名 → 配置 DNS（推荐 CNAME 到 `cname.vercel-dns.com`）

---

### 五、腾讯 EdgeOne 全球区

使用 `adapter-static`，输出到 `build/` 目录。通过 `edgeone.json` 配置构建和 HTTP 头。
...
> 每次 `git push` 自动构建并部署到全球边缘节点。

#### 自定义域名

EdgeOne 控制台 → 站点 → 域名管理 → 添加域名 → 配置 DNS（CNAME 到 EdgeOne 提供的域名）

---

### 部署平台对比

| 平台 | 适配器 | 输出目录 | 构建命令 | 部署命令 | CLI 工具 |
|------|--------|----------|----------|----------|----------|
| Cloudflare Workers | `adapter-cloudflare` | `.svelte-kit/cloudflare/` | `pnpm build:cloudflare` | `pnpm deploy:cloudflare` | wrangler |
| Cloudflare Pages | `adapter-static` | `build/` | `pnpm build:cf-pages` | `pnpm deploy:cf-pages` | wrangler |
| Netlify | `adapter-static` | `build/` | `pnpm build:netlify` | `pnpm deploy:netlify` | netlify-cli |
| Vercel | `adapter-static` | `build/` | `pnpm build:vercel` | `pnpm deploy:vercel` | vercel |
| EdgeOne 全球区 | `adapter-static` | `build/` | `pnpm build:edgeone` | `pnpm deploy:edgeone` | @tencent/edgeone |
...
| 头部 | 值 | 适用资源 |
|------|-----|----------|
| `X-Frame-Options` | `DENY` | 所有资源 |
| `X-Content-Type-Options` | `nosniff` | 所有资源 |
| `Referrer-Policy` | `strict-origin-when-cross-origin` | 所有资源 |
| `Cache-Control` | `public, max-age=31536000, immutable` | `*.avif` / `*.css` / `*.js` |
| `Cache-Control` | `public, max-age=604800, immutable` | `favicon.svg` |
...
| `wrangler.toml` | Cloudflare Workers 配置（Worker 入口 + 静态资源 + 安全头 + 重定向） |
| `netlify.toml` | Netlify 配置（构建命令 + 发布目录 + 安全头 + 缓存策略） |
| `vercel.json` | Vercel 配置（构建命令 + 输出目录 + 安全头 + 缓存策略 + SPA 回退） |
| `edgeone.json` | 腾讯 EdgeOne 全球区配置（构建命令 + 输出目录 + 安全头 + 缓存策略） |
| `static/_headers` | Cloudflare Pages 自定义 HTTP 响应头（自动复制到 `build/`） |
| `static/_redirects` | Cloudflare Pages SPA 路由回退规则（自动复制到 `build/`） |
