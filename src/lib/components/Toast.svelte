<script lang="ts">
	...
	function iconClass(type: string) {
		return type === 'success' ? 'text-green-500' : type === 'error' ? 'text-red-500' : 'text-blue-500';
	}
</script>

<div aria-live="polite" class="sr-only">
	{#each toast.items as item (item.id)}
		<p>{item.title}{item.description ? `: ${item.description}` : ''}</p>
	{/each}
</div>
...
{#each toast.items as item, i (item.id)}
	...
	>
		<button
			...
			class="absolute top-2 right-2 rounded-md p-1 hover:bg-black/5 dark:hover:bg-white/10"
			aria-label="关闭"
		>
			<Icon icon="mdi:close" class="size-4 text-muted-foreground" />
		</button>
		<div class="flex items-start gap-3 p-4 pr-8">
			<Icon icon={typeConfig[item.type].icon} class="size-5 shrink-0 {iconClass(item.type)}" />
