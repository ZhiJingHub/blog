<script lang="ts">
	...
	onMount(() => {
		if ('queryLocalFonts' in window) fontApiSupported = true;
	});
</script>
...
<Card>
	...
	<CardContent class="space-y-4">
		...
		<div class="space-y-2">
			...
			<Slider type="single" value={fontWeight} onValueChange={(v: number) => (fontWeight = v)} min={100} max={900} step={100} />
		</div>
		<div class="space-y-4 border-t pt-4">
			<Label>自定义字体</Label>
			<div>
				<input type="file" accept=".ttf,.otf,.woff,.woff2" onchange={onFontUpload} class="hidden" id="font-upload" />
				<Label for="font-upload" class="flex h-16 w-full cursor-pointer items-center justify-center rounded-lg border-2 border-dashed hover:border-primary">
					...
					<div class="flex items-center justify-between">
						...
						{#if systemFonts.length === 0}
							<Button variant="outline" size="sm" onclick={loadSystemFonts} disabled={isLoadingFonts}>
								{#if isLoadingFonts}
									<Icon icon="mdi:loading" class="mr-2 h-4 w-4 animate-spin" />加载中...
								{:else}
									<Icon icon="mdi:folder-open" class="mr-2 h-4 w-4" />获取系统字体
								{/if}
							</Button>
						{/if}
					</div>
					{#if systemFonts.length > 0}
						...
						<div class="max-h-48 overflow-y-auto rounded-lg border">
							<div class="divide-y">
								{#each paginatedFonts as font}
									<button onclick={() => onSystemFontSelect(font)} class="w-full px-3 py-2 text-left transition-colors hover:bg-accent {customFontName === font ? 'bg-primary/10 font-medium' : ''}" style="font-family: '{font}', sans-serif;">
										{font}
									</button>
								{/each}
							</div>
						...
						{#if totalPages > 1}
							<div class="flex items-center justify-between text-sm">
								<span class="text-muted-foreground">第 {currentPage} / {totalPages} 页，共 {filteredFonts.length} 个字体</span>
			...
			{:else}
				<p class="text-xs text-muted-foreground">您的浏览器不支持本地字体访问 API</p>
			{/if}
		</div>
	</CardContent>
</Card>
