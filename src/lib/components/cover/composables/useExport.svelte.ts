...
interface ExportContext {
	getSvgContainer: () => SVGSVGElement | undefined;
...
export function useExport(ctx: ExportContext) {
	...
	async function doExport() {
		...
		try {
			...
			if (!svgContainer) {
				exportStatus = { type: 'error', message: '预览区域未初始化，请刷新页面后重试。' };
				return;
			}
			...
			if (bgImage) {
				...
				if (imgEl && bgImage.startsWith('blob:')) {
					try {
						...
						exportStatus = { type: 'error', message: '背景图片转换失败，请尝试重新上传背景图片后导出。' };
						return;
					}
				}
			}
			...
			if (localIcon) {
				...
				if (localImgEl && localIcon.startsWith('blob:')) {
					try {
						...
						exportStatus = { type: 'error', message: '本地图标转换失败，请尝试重新上传图标后导出。' };
						return;
					}
				}
			}
			...
			exportStatus = { type: 'success', message: `导出成功，共 ${exportedCount} 个文件。` };
		} catch (e) {
